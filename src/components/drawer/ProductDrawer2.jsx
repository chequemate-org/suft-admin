...
const ProductDrawer3 = ({ id }) => {
  ...
  const removeColor = (colorToRemove) => {
    setColor(color.filter((color) => color.name !== colorToRemove));
  };
...
const ProductDrawer2 = ({ id, title, fetchProduct, product }) => {
  const { t } = useTranslation();
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success(
          uuid ? "Product updated successfully!" : "Product added successfully!"
        );
        resetForm();
      } else {
